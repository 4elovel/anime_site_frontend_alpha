  ...
  return (
    <div className="w-80 flex-col gap-4 p-4 bg-white dark:bg-zinc-900 rounded-xl shadow-xl border">
      <div className="flex flex-col gap-2">
        ...
        {synopsis && (
          <div className="text-muted-foreground mb-2 line-clamp-4 text-sm">{synopsis}</div>
        )}
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          {media_type && <span>{media_type}</span>}
          {status && <span className="rounded bg-gray-200 px-2 py-0.5 text-xs font-semibold text-gray-700">{status}</span>}
          {typeof episodes_released === "number" && typeof episodes_total === "number" && (
            <span>Епізоди: {episodes_released} / {episodes_total}</span>
          )}
        </div>
        {genres.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-2">
            ...
            {genres.map((genre, i) => (
              <span key={genre.slug} className="underline text-primary">
                {genre.name_ua}{i + 1 !== genres.length && <span>, </span>}
              </span>
            ))}
          </div>
        )}
