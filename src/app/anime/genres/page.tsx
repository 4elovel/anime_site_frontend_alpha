  ...
  useEffect(() => {
    ...
    async function fetchGenres() {
      ...
      try {
        ...
        const res = await fetch(`${API_BASE_URL}genres?sort=${backendSort}&direction=${direction}&page=${page}`);
            ...
            >
              <div className="flex flex-col gap-12 w-full">
                {genres.map((genre) => {
                  ...
                  let images: string[] = [];
                  if (Array.isArray(genre.anime_posters)) images = [...genre.anime_posters];
                  if (images.length < 5 && Array.isArray(genre.person_avatars)) {
                    images = images.concat(genre.person_avatars.slice(0, 5 - images.length));
                  }
